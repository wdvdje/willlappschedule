...
(function () {
  ...
  function importFile() {
    ...
    inp.addEventListener('change', function () {
      ...
      reader.onload = function (e) {
        ...
        catch (_) { alert('iCloud Sync: invalid JSON file.'); }
      };
      reader.readAsText(f);
    });
    inp.click();
  ...
  /* Silent overwrite — bypasses confirm() dialogs for background sync */
  function _applySilent(importData) {
    if (typeof applyImportData !== 'function') return;
    var prev = window.confirm;
    window.confirm = function () { return true; };
    try {
      applyImportData(importData, 'overwrite');
    } finally {
      window.confirm = prev;
    }
  }
   ...
   * These functions are defined in app.js at the global scope. */
